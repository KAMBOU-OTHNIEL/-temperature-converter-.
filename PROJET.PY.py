...
if image_path_2022 is None or image_path_2023 is None:
    exit(1)
...
# Visualiser la images corrigées
plt.figure(figsize=(10, 5))
plt.subplot(1, 2, 1)
plt.imshow(image_2022_corrected[0], cmap='gray')
plt.title('Image 2020 Corrigée')
...
heat_data = [[lat, lon] for lat, lon in zip(change_coords[:, 0], change_coords[:, 1])]

# Convertir les coordonnées des pixels en coordonnées géographiques
transform = profile_2022['transform']
heat_data_geo = [(transform * (coord[1], coord[0]))[::-1] for coord in heat_data]
...
<p>Nombre total de changements détectés : {change_coords}</p>
